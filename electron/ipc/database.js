...
export const setupDatabaseHandlers = () => {
           ...
           WHERE oi.order_id = ?`,
                ...
                for (const item of orderItems) {
                    ...
                    if (itemUpdate.changes > 0) {
                        ...
                        executeUpdate(
                            `INSERT INTO stock_movements (product_id, quantity_change, movement_type, reason, reference_id)
               VALUES (?, ?, 'ORDER_SHIPPED', ?, ?)`,
                            [item.product_id, -item.quantity, `Order ${order.order_number} shipped`, id]
                        );
            ...
            return executeUpdate('UPDATE orders SET status = ? WHERE id = ?', [status, id]);
        });
    });
    ...
    ipcMain.handle('orders:getByStatus', async (event, status) => {
        return executeQuery('SELECT * FROM orders WHERE status = ? ORDER BY created_at DESC', [status]);
    });
    ...
    ipcMain.handle('returns:create', async (event, returnData) => {
        const result = executeUpdate(
            `INSERT INTO returns (order_id, return_type, reason, inspection_notes, inspection_checklist, images, status)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
            [returnData.order_id, returnData.return_type, returnData.reason, returnData.inspection_notes, returnData.inspection_checklist, returnData.images, returnData.status || 'PENDING']
        );
        return { id: result.lastInsertRowid };
    });
    ...
    ipcMain.handle('returns:update', async (event, id, returnData) => {
        return transaction(() => {
            if (returnData.status === 'APPROVED' && returnData.restock_status === 'YES') {
                const returnRecord = executeQueryOne('SELECT * FROM returns WHERE id = ?', [id]);
           ...
           WHERE oi.order_id = ?`,
             ...
             WHERE id = ? AND stock_deducted = 1 AND stock_returned = 0`,
               ...
               VALUES (?, ?, 'RETURN_RESTOCK', ?, ?)`,
                            [item.product_id, item.quantity, `Return ${id} approved`, returnRecord.order_id]
                        );
            ...
            if (updateFields.length > 0) {
                ...
                return executeUpdate(
                    `UPDATE returns SET ${updateFields.join(', ')} WHERE id = ?`,
                    updateValues
                );
            }
    ...
    ipcMain.handle('stock:addMovement', async (event, movement) => {
        const result = executeUpdate(
            `INSERT INTO stock_movements (product_id, quantity_change, movement_type, reason, reference_id, staff_id)
       VALUES (?, ?, ?, ?, ?, ?)`,
            [movement.product_id, movement.quantity_change, movement.movement_type, movement.reason, movement.reference_id, movement.staff_id]
        );
        return { id: result.lastInsertRowid };
    });
    ...
    ipcMain.handle('audit:addLog', async (event, log) => {
        const result = executeUpdate(
            ...
            [log.staff_id, log.action, log.entity_type, log.entity_id, log.details]
        );
        return { id: result.lastInsertRowid };
    });
    ...
    ipcMain.handle('packing_evidence:create', async (event, evidence) => {
        const result = executeUpdate(
            `INSERT INTO packing_evidence (order_id, video_id, checklist_product_correct, checklist_quantity_correct, checklist_sealing_done, photo_before_seal, photo_after_seal)
             VALUES (?, ?, ?, ?, ?, ?, ?)`,
            [
                evidence.order_id,
                evidence.video_id,
    ...
    ipcMain.handle('packing_evidence:getByOrderId', async (event, orderId) => {
        return executeQueryOne('SELECT * FROM packing_evidence WHERE order_id = ? ORDER BY created_at DESC LIMIT 1', [orderId]);
    });
        ...
        return transaction(() => {
            ...
            for (const movement of movements) {
                ...
                const movementResult = executeUpdate(
                    `INSERT INTO inventory_movements (product_id, quantity, direction, reason, notes, performed_by)
                     VALUES (?, ?, ?, ?, ?, ?)`,
                    [movement.product_id, movement.quantity, movement.direction, movement.reason, movement.notes, movement.performed_by]
                );
        ...
        try {
            ...
            try {
                ...
                return transaction(() => {
                    ...
                    executeUpdate('DELETE FROM packing_evidence');
                    ...
                    executeUpdate('DELETE FROM stock_movements');

                    console.log('[Database] Clearing inventory_movements...');
                    executeUpdate('DELETE FROM inventory_movements');

                    console.log('[Database] Clearing audit_logs...');
                    executeUpdate('DELETE FROM audit_logs');

                    console.log('[Database] Clearing videos...');
                    executeUpdate('DELETE FROM videos');

                    console.log('[Database] Clearing returns...');
                    executeUpdate('DELETE FROM returns');
                    ...
                    console.log('[Database] All test data cleared successfully');
